...
def decompress(fname):
    ...
    if new_type:
        new_plus_type = get_new_name('{}.{}'.format(new_name, new_type))
...
def flatten_folder(base_dir):
    ...
    for root, dirs, files in os.walk(base_dir):
        if root != base_dir:
            for name in files:
                new_name = get_new_name('{}/{}'.format(base_dir, name))
                shutil.move('{}/{}'.format(root, name), new_name)
    for root, dirs, files in os.walk(base_dir):
        for d in dirs:
...
def get_new_name(fname):
    ...
    count = 1
    while True:
        count_val = '({})'.format(count)
        if is_dir:
            temp_name = fname + count_val
        else:
            split_index = fname.rfind(".")
            temp_name = '{}{}{}'.format(fname[:split_index], count_val,
                                        fname[split_index:])
